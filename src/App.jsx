...
const createLessonReplayBoardState = (items) => {
  ...
  items.forEach((item, index) => {
    ...
    if (!id) return;
    signaturesById.set(id, JSON.stringify(item));
    indexesById.set(id, index);
  });
  return { items, signaturesById, indexesById };
};
...
const createLessonReplayBoardPayload = (items, previousState, forceSnapshot = false) => {
  ...
  nextState.items.forEach((item, index) => {
    ...
    if (!id) return;
    if (
      previousState.signaturesById.get(id) !== nextState.signaturesById.get(id)
      || previousState.indexesById.get(id) !== index
    ) {
      upserts.push({ index, item });
    }
  });
  const removedIds = [];
  previousState.signaturesById.forEach((_, id) => {
    if (!nextState.signaturesById.has(id)) removedIds.push(id);
  });
  if (upserts.length === 0 && removedIds.length === 0) return { payload: null, state: nextState };
  return {
    payload: { mode: 'delta', upserts, removedIds },
    state: nextState,
  };
};
...
const BoardSection = ({
  ...
  const flushLessonReplayBoardSnapshot = useCallback(() => {
    ...
    const items = lessonReplayPendingBoardRef.current;
    lessonReplayPendingBoardRef.current = null;
    if (!items || typeof lessonReplayEventRef.current !== 'function') return false;
    const now = Date.now();
    const forceSnapshot = (
      !lessonReplayLastBoardStateRef.current
      || now - lessonReplayLastBoardKeyframeAtRef.current >= LESSON_REPLAY_BOARD_KEYFRAME_MS
    );
    const checkpoint = createLessonReplayBoardPayload(
      items,
      lessonReplayLastBoardStateRef.current,
      forceSnapshot
    );
  ...
  const scheduleLessonReplayBoardSnapshot = useCallback((items, delayMs = LESSON_REPLAY_BOARD_CHECKPOINT_MS) => {
    if (typeof window === 'undefined') return;
    lessonReplayPendingBoardRef.current = Array.isArray(items) ? items : [];
    if (lessonReplayBoardTimerRef.current && Number(delayMs) > 0) return;
  ...
  useEffect(() => {
    ...
    const updateItems = (event, transaction) => {
      ...
      if (
        ...
        && shouldRecordReplay
        && previousItems.length > nextSnapshot.nextItems.length
      ) {
        scheduleLessonReplayBoardSnapshot(previousItems, 0);
        flushLessonReplayBoardSnapshot();
      }
...
const DashboardLayout = ({ user, onLogout, progress, onUpdateProgress, theme, onThemeToggle, onUserUpdated }) => {
  ...
  const handleOpenTask = (taskNumber, levelId, targetQuestions, options = {}) => {
    ...
    const nextTask = {
      ...
      questionIndex: Number.isFinite(Number(options?.questionIndex))
  ...
  const quickHomeworkNextTask = pickNextHomeworkQuickTask(
    ...
    quickHomeworkSession.currentTask?.key
  );
  const quickHomeworkPreviewTask = quickHomeworkSession.status === 'idle'
  ...
  const openQuickHomeworkTask = (task, { reset = false } = {}) => {
    ...
    handleOpenTask(
      task.taskNumber,
      task.levelId,
      task.questionNumber ? [task.questionNumber] : null,
      {
        quickHomework: true,
        questionIndex: task.isPython && task.questionNumber
          ? Math.max(0, task.questionNumber - 1)
          : null,
      }
    );
  };
  ...
  const handleResumeQuickHomework = () => {
    const task = quickHomeworkSession.currentTask
      || pickNextHomeworkQuickTask(quickHomeworkQueue, quickHomeworkSession.completedKeys);
    openQuickHomeworkTask(task);
  };

  const handleContinueQuickHomework = () => {
    ...
    if (!nextTask) {
      setQuickHomeworkSession((current) => ({ ...current, status: 'done', currentTask: null }));
      return;
    }
    openQuickHomeworkTask(nextTask);
  };
  ...
  const handlePauseQuickHomework = () => {
    setQuickHomeworkSession((current) => {
      ...
      const nextTask = pickNextHomeworkQuickTask(
        ...
        current.currentTask?.key
      );
      return {
        ...current,
        status: nextTask ? 'paused' : 'done',
  ...
  const handleQuickHomeworkTaskSolved = (payload = {}) => {
    ...
    const completion = payload?.options && typeof payload.options === 'object' ? payload.options : {};
    const currentTask = quickHomeworkSession.currentTask;
    if (completion.quickQuestionSolved !== true) return false;
    if (
      completion.taskNumber != null
      && Number(completion.taskNumber) !== Number(currentTask.taskNumber)
    ) {
      return false;
    }
    if (
      completion.levelId
      && String(completion.levelId) !== String(currentTask.levelId)
    ) {
      return false;
    }
    if (
      currentTask.questionId
      && completion.solvedQuestionId
      && String(completion.solvedQuestionId) !== String(currentTask.questionId)
    ) {
      return false;
    }
    if (
      !currentTask.questionId
      && currentTask.questionNumber
  ...
  const shouldShowGoalBlock = user.role === 'student'
    ...
    && !goalState.completed
    && goalGoals.length > 0;
  const shouldShowQuickHomeworkCelebration = user.role === 'student'
    && !['review', 'collab', 'board', 'call', 'chat'].includes(view)
    && (
      quickHomeworkSession.status === 'celebrate'
      || quickHomeworkSession.status === 'complete'
    );
      ...
      <div className={`main-shell relative flex-1 flex flex-col app-h overflow-hidden ${desktopNavCollapsed ? 'desktop-main-shifted' : ''}${isStudentChatView ? ' main-shell--student-chat' : ''}`}>
        ...
        >
          ...
          {shouldShowQuickHomeworkCelebration && !studentTourActive && (
            <HomeworkQuickStart
              celebrationOnly
              status={quickHomeworkSession.status}
              availableCount={quickHomeworkQueue.length}
              ...
              onPause={handlePauseQuickHomework}
            />
          )}
          ...
          {view === 'schedule' && user.role === 'student' && (
            <StudentTodayOverview
              ...
              chatUnreadCount={studentChatNavUnreadTotal}
              quickHomeworkStatus={quickHomeworkSession.status}
