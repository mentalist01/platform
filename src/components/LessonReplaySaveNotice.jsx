...
export default function LessonReplaySaveNotice({ role, error, onRetry, onDownload, onDiscard }) {
  ...
  return (
    <details className="lesson-replay-save-notice mx-3 my-1.5 min-h-0 shrink-0 overflow-y-auto rounded-lg border border-amber-200 bg-amber-50 text-xs text-amber-950" style={{ maxHeight: '30%' }}>
      <summary className="flex cursor-pointer items-center gap-2 px-3 py-2 font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-600">
        <span role="status">Запись урока: требуется сохранение</span>
        <span className="font-normal underline underline-offset-2">Подробнее</span>
        {discardable && <button
          type="button"
          onClick={(event) => { event.preventDefault(); event.stopPropagation(); void onDiscard(); }}
          className="ml-auto rounded-md border border-amber-400 px-2 py-1 font-semibold"
        >
          Убрать
        </button>}
      </summary>
