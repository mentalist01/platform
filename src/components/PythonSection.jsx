...
const PythonSection = ({
  ...
  const [activeTheoryLaunch, setActiveTheoryLaunch] = useState(null);
  ...
  useEffect(() => {
    ...
    setActiveTheoryLaunch(openTask.openTheory === true
      ? {
          open: true,
          type: String(openTask.theoryType || '').trim(),
          subsectionId: String(openTask.subsectionId || '').trim() || null,
        }
      : null);
    ...
    <div className={`python-learning-shell python-learning-shell--refined ${role === 'student' ? 'python-learning-shell--student' : 'python-learning-shell--teacher'} space-y-4 md:space-y-5 animate-fadeIn`}>
      ...
      {role === 'teacher' && (
        <div ref={teacherTaskToolsRef}>
          <Card className={`python-task-builder python-mobile-panel python-mobile-panel--task space-y-4 border-purple-200/60 bg-gradient-to-br from-white via-white to-purple-50/40 ${showTeacherTaskToolsMobile ? 'python-mobile-panel--open' : 'python-mobile-panel--closed'}`}>
          <div className="python-task-builder__hero rounded-2xl border border-white/10 bg-slate-950/30 p-4 shadow-[0_14px_30px_rgba(15,23,42,0.16)]">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
              <div className="space-y-2">
                <div className="python-task-builder__eyebrow inline-flex items-center rounded-full border border-violet-400/30 bg-violet-500/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-violet-200">
                  Конструктор Python
                </div>
                <div>
                  <h3 className="python-task-builder__title text-xl font-black text-white">
                    {editingQuestionId ? 'Редактирование задачи' : 'Добавление задачи'}
                  </h3>
                  <p className="python-task-builder__description mt-1 max-w-2xl text-sm text-slate-300">
                    Сначала создайте или выберите карточку, затем соберите подразделы и только после этого добавляйте задачу с тестами.
                  </p>
                </div>
              </div>
              <div className="w-full xl:w-[360px]">
                <div className="python-task-builder__field-label mb-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">
                  Активная карточка
                </div>
                <select
          ...
          <div className="python-task-builder__step-card python-task-builder__step-card--card rounded-3xl border border-purple-200/20 bg-gradient-to-br from-purple-500/10 via-slate-950/20 to-violet-500/8 p-4 md:p-5 space-y-4 shadow-[0_18px_36px_rgba(88,28,135,0.18)]">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="python-task-builder__step-label text-[11px] font-bold uppercase tracking-[0.18em] text-cyan-200">Шаг 1</div>
                <h4 className="python-task-builder__step-title mt-1 text-lg font-black text-white">Карточка</h4>
                <p className="python-task-builder__step-description mt-1 max-w-2xl text-sm text-slate-300">
          ...
          <div className="python-task-builder__step-card python-task-builder__step-card--subsections rounded-3xl border border-violet-200/20 bg-gradient-to-br from-violet-500/10 via-slate-950/20 to-fuchsia-500/8 p-4 md:p-5 space-y-4 shadow-[0_18px_36px_rgba(76,29,149,0.18)]">
            <div className="flex flex-col gap-1">
              <div className="python-task-builder__step-label text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-200">Шаг 2</div>
              <h4 className="python-task-builder__step-title text-lg font-black text-white">Подразделы карточки</h4>
              <div className="python-task-builder__step-description text-sm text-slate-300">Подразделы помогают разбить одну карточку на понятные группы задач: например, “Генераторы”, “Списки”, “Строки”.</div>
            </div>
            <div className="flex flex-wrap gap-2">
              {manageSubsections.length === 0 ? (
                <div className="python-task-builder__empty-state rounded-2xl border border-dashed border-violet-300/20 bg-slate-950/20 px-3 py-3 text-sm text-slate-300">
          ...
          <div className="python-task-builder__step-card python-task-builder__step-card--task rounded-3xl border border-violet-200/20 bg-gradient-to-br from-violet-500/8 via-slate-950/20 to-fuchsia-500/8 p-4 md:p-5 space-y-4 shadow-[0_18px_36px_rgba(88,28,135,0.18)]">
            <div className="flex flex-col gap-1">
              <div className="python-task-builder__step-label text-[11px] font-bold uppercase tracking-[0.18em] text-violet-200">Шаг 3</div>
              <h4 className="python-task-builder__step-title text-lg font-black text-white">{editingQuestionId ? 'Редактирование задачи' : 'Задача внутри карточки'}</h4>
              <div className="python-task-builder__step-description text-sm text-slate-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_260px] gap-3">
              <div>
                <label className="python-task-builder__field-label block text-xs font-bold text-gray-400 uppercase mb-1">Стартовый код</label>
                <textarea
              ...
              <div className="python-task-builder__subsection-field rounded-2xl border border-white/10 bg-slate-950/25 p-3">
                <label className="python-task-builder__field-label block text-xs font-bold text-gray-400 uppercase mb-1">Подраздел задачи</label>
                <select
            ...
            <div className="python-task-builder__tests rounded-2xl border border-white/10 bg-slate-950/25 p-3.5 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
                  <div className="python-task-builder__field-label text-xs font-bold text-gray-400 uppercase">Тесты задачи</div>
                  <div className="python-task-builder__hint mt-1 text-xs text-slate-400">Каждый тест состоит из входных данных и ожидаемого вывода.</div>
                </div>
              ...
              {testsFileName && (
                <div className="python-task-builder__hint text-[11px] text-gray-400">Файл: {testsFileName}</div>
              )}
              <div className="space-y-2">
            ...
            <div className="python-task-builder__save-row flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-2">
            ...
            <div className="python-task-builder__existing rounded-2xl border border-white/10 bg-slate-950/20 p-3.5 space-y-3">
              <div>
                <div className="python-task-builder__field-label text-xs font-bold uppercase tracking-wide text-gray-400">Уже добавленные задачи</div>
                <div className="python-task-builder__hint mt-1 text-xs text-slate-400">Быстрый список всего, что уже лежит в выбранной карточке.</div>
              </div>
          ...
          >
            {theoryType === 'text' ? (
              ...
              <div className="space-y-3">
                ...
                <p className="text-[11px] text-gray-400">Вставьте ссылку из Rutube или код вставки плеера. Для видео «только по ссылке» вставляйте полную ссылку вместе с ключом ?p= — без него плеер не откроется.</p>
              </div>
            ) : (
      ...
      {role === 'student' && activeTask && (
        <PythonTestModal
          ...
          initialShowTheory={Boolean(activeTheoryLaunch?.open)}
          initialTheoryType={activeTheoryLaunch?.type || ''}
